...
export default function DateRangeSelector({ availableDates, range, onChange }) {
  ...
  const presets = [
    ...
    { label: "3 Day", days: 3 },
    { label: "7 Day", days: 7 },
    { label: "15 Day", days: Math.min(15, availableDates.length) },
  ];
  ...
  return (
    <div className="mb-6 border border-ink-700 rounded-lg bg-ink-900 p-4">
      <div className="flex flex-col lg:flex-row gap-4 lg:items-center">
        {/* Preset buttons — the central interaction on this site, sized and
            weighted to match, and stretching to fill the row rather than
            sitting as a small compact control off to one side. */}
        ...
        {/* Custom range — secondary to the presets, sits in the remaining
            row space rather than competing for the same visual weight. */}
      ...
      <p className="text-paper/30 text-[11px] font-body mt-3">
        Data available for the last {availableDates.length} trading days
      </p>
    </div>
  );
}
